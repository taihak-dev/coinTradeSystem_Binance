...
def simulate_futures_with_db(
        ...
        # --- 👇 V2 전용 파라미터 추가 👇 ---
        slippage_pct: float = 0.0005  # (0.05% 슬리피지)
        # --- 👆 ---
):
    logging.info(f"--- ⏱️ V2 선물(OHLC) 백테스트 시작: {market}, 기간: {start} ~ {end} ---")
    logging.info(f"--- 레버리지: {leverage}x, 초기 자본: {initial_cash:,.2f} USDT, 슬리피지: {slippage_pct * 100:.3f}% ---")
    ...
    for i, row in df_candles.iterrows():
        ...
        # --- 1. 청산 검사 (V2: '저가(Low)' 기준으로 검사) ---
        if market in position:
            pos_data = position[market]
            # 가장 불리한 '저가'로 포지션 가치 평가
            ...
            if available_margin < maintenance_margin_needed:
                # --- (청산 발생 시 로직은 V1과 동일: 초기화 후 'continue') ---
        ...
        if market in position:
            ...
            if price_high >= target_sell_price:
                ...
                # V2: 매도 체결 시, 이번 1분봉에서 추가 매수는 없다고 가정하고 건너뜀
                # (참고: 매수/매도 동시 체결 로직은 더 복잡하므로 여기서는 매도 우선으로 처리)
                continue

                # --- 3. 매수 전략 실행 (V2: '저가(Low)' 기준) ---
        ...
        # V2: '종가'가 아닌 '저가'를 기준으로 전략 모듈 호출
        new_buy_orders_df = generate_buy_orders(setting_df, buy_log_df, {market: price_low}, sim_holdings,
                                                available_margin)
        if not new_buy_orders_df.empty:
            buy_log_df = pd.concat([buy_log_df, new_buy_orders_df], ignore_index=True)

        # --- 4. 매수 주문 체결 로직 (V2: '저가(Low)' 기준 및 슬리피지) ---
        ...
        # --- 5. 계좌 상태 및 로그 기록 (V2: '종가(Close)' 기준) ---
        # (청산 검사/매매는 Low/High로 했지만, 하루의 마감 평가는 '종가'로 합니다)
        if market in position:
            pos_data = position[market]
        ...
        segment_logs.append({
            "시간": now, "종가": price_close,  # V2: '종가' 명시
            "신호": " / ".join(events) if events else "보유 중",
    ...
    if save_full_log:
        ...
        try:
            ...
            filename = f"FT_V2_시뮬_{market}_{leverage}x_LAST_SEGMENT_{datetime.now().strftime('%Y%m%d_%H%M:%S')}.csv"
